...
- **Email confirmation links fail on localhost** — In Supabase: **Authentication → URL Configuration**, add your exact app URL to **Redirect URLs** (e.g. `http://127.0.0.1:5500/home.html` or `http://localhost:8080/home.html`, matching your dev server). The app now sends `emailRedirectTo` to `home.html` next to whatever page you signed up from. For quick local testing, disable **Confirm email** under **Authentication → Providers → Email**. Don’t open the site as `file://`; use a local HTTP server so redirects work.
